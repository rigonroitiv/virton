...
import {
  getApartmentIdModalApartmentId,
  getApartmentIdModalId,
  getApartmentIdModalState,
...
const AdmApartmentIdModal = () => {
  ...
  function updateApartment({ id, newId }) {
    return axiosInstance.put(
      `${urlObjects}/update/apartmentId?id=${id}&apartmentId=${newId}`
    );
  }
  ...
  const handleSubmit = async () => {
    ...
    if (id !== null) {
      ...
      updateApartment({ id: id, newId: input2 })
        .then((res) => {
